...
> **Work in progress:** this port is usable for the modules listed below, but
> it is not yet a complete Qt 6 replacement for the original Qt Script
> Generator distribution.

This repository ports the generated QtScript bindings used by QCAD and Blink
from Qt 5 to Qt 6. The current milestone builds the `qt.core`,
`qt.gui`, `qt.network`, `qt.sql`, `qt.widgets`, `qt.printsupport`, and
`qt.uitools` QtScript extensions and the `qs_eval` functional-test executable
with Qt 6.9.2/MSVC x64. The evaluator is linked to the ported QtScriptTools
module and includes a headless debugger attach/widget smoke test.

The repository currently contains the generated binding sources and the
handwritten Qt 6 compatibility/build layer. It does not yet contain a Qt 6
port of the generator executable itself, so API additions are maintained in
the generated sources for now.
...
which is itself a fork of the original Qt Labs Qt Script Generator work.

This Qt 6 port retains that ancestry and includes the upstream LGPL license
and Qt LGPL exception. QCAD is not responsible for this experimental port.
...
- The companion
  ...
  installed as an overlay containing both `Qt6::Script` and
  `Qt6::ScriptTools` plus their qmake module metadata.
- Visual Studio 2022 with the x64 C++ toolchain.

The required QtScript port is not part of stock Qt 6. The bindings depend on
our separate QtScript/QtScriptTools port and do not depend on Qt5Compat.

All paths are parameters. The script defaults match the environment used for
development, while another installation can be selected on the command line.
Build products stay below the repository directory.
...
written below `qtbindings\qs_eval`. `test.ps1` runs the core, GUI, loopback
Network, in-memory SQLite, offscreen widget, PDF PrintSupport, and in-memory
Designer UI smoke tests through that evaluator. Each module also has a
regression smoke script covering value conversion, overloads, state changes,
and failure/cleanup paths (for example leap-day `QDateTime` arithmetic,
transaction rollback, model/view data, cookie parsing, PDF page geometry, and
malformed `.ui` handling). Logs are written to `logs`; each evaluator process
has a timeout so a binding or cleanup hang is reported with its captured output.
The Core generated sources keep `QDate`, `QTime`, and `QDateTime` return values
on their QtScript QVariant wrappers instead of Qt 6's built-in JavaScript-Date
conversion; this preserves the generated value prototypes during chained calls.
Use `-EvaluatorTimeoutMilliseconds` with `test.ps1` to adjust the default
30-second per-process limit on slower machines.
...
The port has no Qt Core5Compat build or runtime dependency. `test.ps1` rejects
source references to that module and inspects every selected plugin and
evaluator import table before running the functional tests.
...
The Core, GUI, Network, SQL, Widgets, PrintSupport, and UI Tools extensions
...
local `QStringConverter`-backed `QTextCodec` facade, date/time APIs, XML
`QStringView` results, container changes, removed enum
aliases, and modern MSVC compilation.
...
for `QTouchEvent.TouchPoint`, `QTabletEvent`, and `QWheelEvent`. Existing event
instances remain available through Qt's QObject/event APIs. `QMatrix` is kept
as a source-compatible alias backed by `QTransform`.
...
This repository intentionally limits the Qt 6 port to the Core, GUI, Network,
SQL, Widgets, PrintSupport, and UI Tools extensions. The following original
binding families are out of scope:

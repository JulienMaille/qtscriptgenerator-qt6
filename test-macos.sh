...
while (($#)); do
    case "$1" in
        ...
        -h|--help)
            echo "Usage: $0 [--qt-prefix PATH] [--qt-script-prefix PATH] [--configuration Debug|Release|All]"
            exit 0
            ;;
...
export DYLD_FRAMEWORK_PATH="$qt_script_prefix/lib:$qt_prefix/lib:${DYLD_FRAMEWORK_PATH:-}"
...
run_evaluator() {
    ...
    if ! "$evaluator" "$@" >"$stdout_log" 2>"$stderr_log"; then

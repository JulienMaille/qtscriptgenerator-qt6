...
static QScriptValue qtscript_create_QPinchGesture_ChangeFlag_class(QScriptEngine *engine, QScriptValue &clazz)
{
    ...
    for (int i = 0; i < 3; ++i) {
        clazz.setProperty(QString::fromLatin1(qtscript_QPinchGesture_ChangeFlag_keys[i]),
            engine->newVariant(QVariant::fromValue(qtscript_QPinchGesture_ChangeFlag_values[i])),
...
{
    QPinchGesture::ChangeFlags result{};
    if ((context->argumentCount() == 1) && context->argument(0).isNumber()) {
        result = QPinchGesture::ChangeFlags::fromInt(context->argument(0).toInt32());
    } else {
...
QScriptValue qtscript_create_QPinchGesture_class(QScriptEngine *engine)
{
    ...
    QScriptValue proto = engine->newVariant(QVariant::fromValue((QPinchGesture*)0));

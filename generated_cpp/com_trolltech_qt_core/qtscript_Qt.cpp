...
{
    if ((value >= Qt::LocalTime) && (value <= Qt::TimeZone))
...
{
    int arg = context->argument(0).toInt32();
    if ((arg >= Qt::LocalTime) && (arg <= Qt::TimeZone))
...
{
    if ((value >= Qt::TextDate) && (value <= Qt::RFC2822Date))
...
{
    int arg = context->argument(0).toInt32();
    if ((arg >= Qt::TextDate) && (arg <= Qt::RFC2822Date))
    ...
    QScriptEngine *engine, QScriptValue &clazz)
{
    ...
    for (int i = 0; i < 5; ++i) {
        clazz.setProperty(QString::fromLatin1(qtscript_Qt_ScrollPhase_keys[i]),
            engine->newVariant(QVariant::fromValue(qtscript_Qt_ScrollPhase_values[i])),
            QScriptValue::ReadOnly | QScriptValue::Undeletable);
    }
    return ctor;
}
